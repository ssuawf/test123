...
//    (16B) -  DMA  
    ...
    // ========================================================================
    // IP fragmentation 대신 독립 UDP 패킷으로 응답 분할
    // 각 UDP 패킷 = [chunk_hdr_t 12B] + [data ≤1460B]
    // response_seq: HV가 매 응답마다 증가시키는 시퀀스 번호
    //   → 수신측이 같은 seq의 chunk만 조립, 다른 seq는 skip
    //   → HV가 같은 요청을 VMEXIT 재진입으로 여러번 처리해도 오염 방지
    // ========================================================================
    struct chunk_hdr_t
    {
        std::uint16_t chunk_index;      // 0-based chunk 번호
        std::uint16_t chunk_total;      // 총 chunk 수
        std::uint32_t total_size;       // 전체 응답 크기 (chunk header 미포함)
        std::uint32_t response_seq;     // 응답 시퀀스 번호 (매 응답 증가)
    };
    static_assert(sizeof(chunk_hdr_t) == 12);

    // chunk당 최대 데이터: UDP payload(1472) - chunk_hdr(12) = 1460
